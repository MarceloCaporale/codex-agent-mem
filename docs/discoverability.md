...
`0.7.0` is the current baseline release for `codex-agent-mem`: a local-first MCP continuity layer for Codex built on SQLite, notify-based capture, hierarchical Definition of Done tracking, deterministic closure control, recent-change deltas, scope-guard retrieval, auto-budget compact AGENTS reinjection, operational-state carry-forward, false-completion guardrails, MCP retrieval, and a local inspection UI.
